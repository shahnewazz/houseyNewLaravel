...
<form action="{{route('admin.pages.widgets.update', ['id' => $page->id])}}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('POST')
    <x-core::card>
...
<script>
    ...
    $(document).on('click', '.repeater-form-add-btn', function(e) {
        ...
        var html = $(this).parent().find('.repeater-form-fields').first().clone();

        var widgetForm = $(this).parent().find('[data-item_count]');

        // Assuming `this` refers to the clicked element
        var parentElement = this.parentNode; // Get the parent element
        var widgetForm = parentElement.querySelector('[data-item_count]'); // Find the element with data-item_count

        if (widgetForm) {
            // Increment the item count and update the attribute
            var repeaterIndex = parseInt(widgetForm.getAttribute('data-item_count'), 10) || 0; // Get current count, default to 0
            widgetForm.setAttribute('data-item_count', repeaterIndex + 1);
        }


        var repeaterIndex = parseInt(widgetForm.data('item_count')); // Get the current repeater index

        // console.log(repeaterIndex);
        var updatedHtml = html.html().replaceAll('[repeater-item-1]', '[repeater-item-' + (repeaterIndex + 1) + ']'); // Update the name attribute
        html.html(updatedHtml); 
        $(this).parent().find(".widget-form .repeater-form-fields-wrapper").append(html); // Append the cloned element


        let repeaterWrapper = $(this).parent().find('.repeater-form-fields-wrapper'); // Get the repeater wrapper


        widgetForm.attr('data-item_count', (repeaterIndex + 1)); // Update the repeater index
